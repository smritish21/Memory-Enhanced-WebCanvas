...
import agent.global_file
import numpy as np 
from steps.eval_steps import *
...
def evaluate(file_path, total_token_cost):
    ...
    df["efficiency_score"] = [s / sc if sc != 0 else np.nan for s, sc in zip(df['steps'], df['step_score'])]
    df["efficiency_score_flip"] = [sc / s if s != 0 else np.nan for s, sc in zip(df['steps'], df['step_score'])]
    df.to_excel("task_base_steps_res.xlsx", index=False)
    ...
    df_evaluate = df[["task_name", "status", "steps", "task_score",
                      "task_score_rate", "step_score", "efficiency_score", "task_near_success","efficiency_score_flip"]]
    ...
    efficiency_score_na = df_evaluate["efficiency_score"].isna().mean()
    average_efficiency_score_flip = df_evaluate["efficiency_score_flip"].mean()
    average_efficiency_score_flip_na = df_evaluate["efficiency_score_flip"].isna().mean()
    if total_token_cost != 0:
        usd_efficiency_score = total_token_cost / key_node_completion_sum

    #Steps result
    output_file_path = "steps/tasks_output_all_tasks_test.json" 
    log_file = '2026-01-25_11-13-49_db_steps.log'
    create_logs(log_file, output_file_path)
    step_dic = get_res(output_file_path)
    ...
    result_dict["average_efficiency_score_fails"] = efficiency_score_na
    result_dict["average_efficiency_score_flip"] = average_efficiency_score_flip
    result_dict["average_efficiency_score_flip_na"] = average_efficiency_score_flip_na
    if total_token_cost != 0:
    ...
    result_dict["steps_required_url"] = step_dic["steps_required_url"]
    result_dict["steps_db_provided_url"] = step_dic["steps_db_provided_url"]
    result_dict["steps_no_url"] = step_dic["steps_no_url"] 
    result_dict["matched_steps"] = step_dic["matched_steps"]
    result_dict["useful_matches"] = step_dic["useful_matches"]
    result_dict["total_steps_skipped"] = step_dic["total_steps_skipped"]
    result_dict["db_retrieval_efficiency_percent"] = step_dic["db_retrieval_efficiency_percent"]
...
#print(evaluate(file_path='', total_token_cost=181.86148035))
