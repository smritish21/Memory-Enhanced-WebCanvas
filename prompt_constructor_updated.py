...
class PlanningPromptConstructor(BasePromptConstructor):
    ...
    def construct(
        ...
        if len(previous_trace) > 0:
            ...
            step_data = prep_data(user_request)
            skip_ans= check_path_llm(user_request,previous_trace,step_data)
            print('LLM ans:- --',skip_ans)
            if skip_ans != 'null':
                ext_data,url = extract_step_info(user_request,skip_ans)
                logger.info('Current URL from DB:- ',url)
                self.prompt_user += f"Here is the step that you can directly skip to, please use the following data to your best. The following data comes from a database where how you performed this task in the past is stored. You can get information from the thought and directly use the URL given here. {ext_data}"
            else:
                if db_mem != "":
                    self.prompt_user += f"History: Here is the history of how you solved a related task in the past. There are thought processes of the task that closely relates to the current task.\n Please follow these thought processes as much as possible for the current task. If these thoughts are not applicable, move on to solving the task as per your knowledge.\n':\n {db_mem}\n"
                if new_url != "":
                    self.prompt_user += f"This is a new URL generated based on the history of related tasks: {new_url}. Please verify your output from previous runs, if this url has already been verified by you then do not use it again. If not, then verify the task output for this URL first. If this URL is not giving the correct result, then move onto finding the correct URL based on your knowledge."
                if cache_mem != "":
                    self.prompt_user += f"You have traversed this website before. Here is what you performed on the website when you visited it in the past {cache_mem}. Use this information to efficiently solve the new task. You can reuse what you did previously based on the thought and action."
            if feedback != "":
