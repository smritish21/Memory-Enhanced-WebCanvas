...
from evaluate.database_wc import *
from evaluate.rag_query import *
from evaluate.genralize_url import classify_url,add_url_to_chroma_db,gen_url
import hashlib
...
class DomMode(InteractionMode):
    ...
    async def execute(self, status_description, user_request, previous_trace, observation, feedback,config, observation_VforD):
        #website = decide_website(user_request)     
        new_url = ''
        cache_mem = ''
        db_mem = ''   
        if(config["files"]["use_database"]):
            db_mem = query_db_emb(user_request)
            logger.info(f"----Querring DB FINISHED----")
        if(config["files"]["use_url_gen"]):
            logger.info(f"----Generating URL----")
            check_url = query_db_url(user_request)
            if check_url:
                new_url = gen_url(check_url,user_request)
            logger.info(f"----Generating URL FINISHED----")
        if(config["files"]["use_dom_hash_matching_with_db"]):
            logger.info(f"----Checking Cache----")
            dom_hash = hashlib.sha256(observation.encode('utf-8')).hexdigest()
            if query_db_dom_hash(dom_hash,user_request):
                #db_task = query_db_dom_hash(dom_hash)
                cache_mem = query_db_dom_hash(dom_hash,user_request)
            #else:
                #cache_mem = query_db_emb(user_request)
            logger.info(f"----Checking Cache FINISHED----")
        '''
        user_query = f"Find tasks related to the task: '{user_request}'."
        
        db_mem= answer_query_ch(user_query,'webcanvas_collection')
        
        db_mem= answer_query(user_query, index_for_rag,data_for_rag)
        
        db_data = []
        if db_mem:
            for task_name, step_url, thought, status in db_mem:
                db_data.append(f"Task: {task_name},URL visited: {step_url},Thought: {thought},Status: {status}")
            logger.info(f"Data fetched from DB : {db_data}")
        else:
            logger.info(f"No data found in DB for this task")'
        '''
        logger.info("** Planning Phase**")
        planning_request = PlanningPromptConstructor().construct(
            user_request, previous_trace, observation, feedback, status_description,db_mem,new_url,cache_mem)
        logger.info(
